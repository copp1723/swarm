...
def initialize_databases(app: Flask):
    ...
    # Initialize async database with better error handling
    try:
        # Check if we're in an existing event loop
        try:
            loop = asyncio.get_running_loop()
            # If we're in a loop, schedule the async init for later
            logger.info("Deferring async database initialization to avoid event loop conflicts")
            return
        except RuntimeError:
        ...
        logger.warning(f"Async database initialization failed, continuing with sync only: {e}")
        # Continue with sync database only - not critical for basic functionality
